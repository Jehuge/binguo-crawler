...
class DouYinClient:
    ...
    def _get_jittered_sleep_time(self, base_time: float) -> float:
        ...
        # 使用时间戳作为伪随机数
        jitter = base_time * jitter_ratio * (((time.time() * 1000) % 100 / 50) - 1)
    ...
    async def _risk_control_sleep(self):
        ...
        # 随机选择间隔时间
        sleep_time = _get_random_sleep(min_sleep, max_sleep)
    ...
    def _get_web_id(self) -> str:
        ...
        # 使用时间的最后几位作为随机数
        pseudo_rand = (t * 7 + 13) % 900000 + 100000
        return str(t) + str(pseudo_rand)
    ...
    async def search_videos(
        ...
        query_params = {
            ...
            "is_filter_search": "0",
            "from_group_id": "7378810571505847586",  # 使用固定值，与 MediaCrawler 保持一致
            "offset": offset,
            "count": "15",
    ...
    async def get_all_comments(
        ...
        while comments_has_more and len(result) < max_count:
            ...
            sleep_time = _get_random_sleep(
                getattr(config, 'CRAWLER_MIN_SLEEP_SEC', 3),
                getattr(config, 'CRAWLER_MAX_SLEEP_SEC', 8),
                getattr(config, 'CRAWLER_USE_JITTER', True),
                getattr(config, 'CRAWLER_JITTER_RATIO', 0.3)
            )
            await asyncio.sleep(sleep_time)
                ...
                for comment in comments:
                    ...
                    if reply_comment_total > 0:
                        ...
                        while sub_comments_has_more and len(result) < max_count:
                            try:
                                # 子评论请求间隔（降低延迟提高效率）
                                await asyncio.sleep(_get_random_sleep(0.3, 0.8, False))
                                ...
                                if "账号可能被风控" in error_msg or "empty" in error_msg.lower():
                                    ...
                                    if retry_count < 3:
                                        wait_time = 1.5 * (retry_count + 1) + _get_random_sleep(0, 0.5, False)  # 线性退避
