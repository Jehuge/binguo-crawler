...
class DouYinCrawler:
    ...
    async def search_videos(
        ...
        while len(videos) < max_count:
            ...
            if page > 0:
                import random
                min_sleep = config.CRAWLER_MIN_SLEEP_SEC
                max_sleep = config.CRAWLER_MAX_SLEEP_SEC
                sleep_time = random.uniform(min_sleep, max_sleep)
                if config.CRAWLER_USE_JITTER:
                    jitter = sleep_time * config.CRAWLER_JITTER_RATIO * (random.random() * 2 - 1)
    ...
    async def crawl_comments(
        ...
        for i, video_id in enumerate(remaining_ids):
            ...
            if i < len(remaining_ids) - 1:
                # 使用带有随机抖动的间隔，而不是固定的 delay
                min_sleep = config.CRAWLER_MIN_SLEEP_SEC
                max_sleep = config.CRAWLER_MAX_SLEEP_SEC
                import random
                actual_delay = random.uniform(min_sleep, max_sleep)
                # 添加抖动
                if config.CRAWLER_USE_JITTER:
                    jitter = actual_delay * config.CRAWLER_JITTER_RATIO * (random.random() * 2 - 1)
                    actual_delay = max(1, actual_delay + jitter)
